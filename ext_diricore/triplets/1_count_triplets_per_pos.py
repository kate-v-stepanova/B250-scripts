...
outdir = os.path.join(project_dir, "analysis/output/ext_diricore/{}/codons_per_pos/{}x.window{}".format(bam_type, x, window))
...
#        codons_df = codons_df.loc[codons_df['transcript'].isin(selected_transcripts)]
        ...
        df1 = df1.drop(['start', 'end', 'seq'], axis=1)
        df1['counts'] = 1
        df2 = df1.groupby(['transcript', 'codon', 'codon_pos', 'codon_end']).sum().reset_index()
        df2['aa'] = aa
